...
cdneat: CoDeepNEAT implementation compatible with the singularity
descentralized optimization network.

Author: Harvey Bastidas       Date: 20170731

Description:

This file is modified from the circuit evolution example of neat-python
the objective is to evolve a topology of modules composed of
deep convolutional and lstm networks by using a bilevel optimization
that uses NEAT to optimize deep net local hyperparametes on each module
individually, and another NEAT to optimize by co-evolution the
global hyperparameters. This technique is called CoDeepNEAT and is
described in the paper "Evolving Deep Neural Networks"
by R, Mikkulainen (2017).

The genomes in the lower layer optimization encode the individual
modules' configuration parameters, while the genomes in the upper layer
encode the global parameters and module network topology.

"""

## Documentación compatible con doxygen
# TODO: ALL: replace genom_lower encodings(keras), replace genom_upper encodings
# Implement cross mutation, reproduction and selection.
#
import os
