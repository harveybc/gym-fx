...
class PooledErrorCompute(object):
    ...
    def simulate(self, nets):
        scores = []
        self.test_episodes = []
        # Eval鷄 cada net en el env actual 
...
def run():
    ...
    # asigna un gen_best para poder cargar los dem谩s desde syn
    ...
    while 1:
        try:
            if temp >= 0:
                ...
                # Lee en pop2 el 煤ltimo checkpoint desde syn
                ...
                if cont['result'][0]['current_block_performance'] > best_fitness:
                    ...
                    if cont_param['result'][0]['parameter_link'] is not None:
                        ...
                        with open('remote_reps', 'wb') as handler:
                            ...
                            handler.close()
                        # carga genom descargado en nueva poblaci贸n pop2
                        with open('remote_reps', 'rb') as f:
                            remote_reps = pickle.load(f)
                        # OP.MIGRATION: Reemplaza el peor de la especie pop1 m谩s cercana por el nuevo chmpion de pop2 como http://neo.lcc.uma.es/Articles/WRH98.pdf
                            ...
                            # Hack: overwrites original genome key with the replacing one
                                ...
                                if closer is not None:
                                    if closer not in remote_reps:
                                        # ADicionada condici贸n para que NO migre el campe贸n, solo los reps (prueba)
                                        ...
                                        #si closer est谩 en remote_reps es porque no hay ningun otro cercano as铆 que lo adiciona
                                        tmp_genom = deepcopy(remote_reps[i])
                                        # Hack: overwrites original genome key with the replacing one
                                        tmp_genom.key = len(pop.population) + 1
                                        pop.population[tmp_genom.key] = tmp_genom
                ...
                #           si closer is in reps
                 ...
                 #               si l tiene m谩s fitness que closer,
                 ...
                 #                      adiciona closer a reps si ya no estaba en reps
                                        if l not in reps:
                                            ...
                                            # Guarda checkpoint de los representatives de cada especie y lo copia a ubicaci贸n para servir v铆a syn.
            ...
            # si validation_score > validation_score_ant incrementa index_t, verifica sus l韒ites e imprime
